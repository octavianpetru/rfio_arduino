...
unsigned int RCSwitch::timingsCopy[RCSWITCH_MAX_CHANGES];
#endif
...
bool RCSwitch::receiveProtocol(const int p, unsigned int changeCount) {
    ...
    for (unsigned int i = 1; i < changeCount && i < 65; i += 2) {
        code <<= 1;
    ...
    if (changeCount > 6) {    // ignore < 4bit values as there are no devices sending 4bit values => noise
		...
		//	RCSwitch::timingsCopy[i] = RCSwitch::timings[i];
		//}
    }
...
void RCSwitch::handleInterrupt() {
  ...
  if (duration > RCSwitch::nSeparationLimit && diff(duration, RCSwitch::timings[0]) < 200) {
    ...
    changeCount--;
    if (repeatCount == 2) {
		receiveProtocol(6, changeCount); // TODO
  ...
  */
  ...
  // if duration < pulseLength -> noise  
  if(duration < 426 * 0.9) {
	  return;
  } else if(changeCount != 0 && diff(RCSwitch::timings[changeCount - 1], 426) < 426 * 0.1) {
	  if(duration < 2130 * 0.9) {
		// previous was a short pulse and this one is shorter than min -> noise
		return;
	  } else if (duration > 2130 * 1.1 && duration < 3834 * 0.9) {
		// previous was a short pulse and this one is longer than min and shorter than max -> noise
		return;
	  }
  }
